...
module.exports.preBuildDevelopment = async () => {
  ...
  const tenantDetailsUrl = "/api/globals/discovery-resorts";
  ...
  const menusHandler = await axios.get(
    envVars.NEXT_PUBLIC_TENANT_API +
      "/api/menus/discovery-resorts-header-menu?include=nodes.children,parentNodes"
  );
  const menus = dataFormatter.deserialize(menusHandler.data);

  const footerMenuHandler = await axios.get(
    envVars.NEXT_PUBLIC_TENANT_API +
      "/api/menus/discovery-resorts-footer-menu?include=parentNodes.children"
  );
  ...
  const restaurantTaxonomiesHandler = await axios.get(
    envVars.NEXT_PUBLIC_TENANT_API +
      "/api/taxonomies/restaurant-category?include=taxonomyTerms"
  );

  const restaurantTaxonomies = dataFormatter.deserialize(
    restaurantTaxonomiesHandler.data
  );
  ...
  generateStaticJson("restaurant-category.json", restaurantTaxonomies);
